...
int main(int argc, char *argv[])
{
  ...
  for (int tt = 0; tt < params.maxIters; tt++)
  {
    ...
    accelerate_flow(params, cells, obstacles, ocl);    // 0.58
    rebound(params, cells, tmp_cells, obstacles, ocl); // 2.004634

    // av_vels[tt] = av_velocity_kernel(params, cells, obstacles, ocl) / (float)*tot_cells; // 4.025352
...
int rebound(const t_param params, t_speed *cells, t_speed *tmp_cells, int *obstacles, t_ocl ocl)
{
  ...
  err = clSetKernelArg(ocl.rebound, 8, sizeof(cl_mem) * (bx + 2) * (by + 2), NULL);
  checkError(err, "setting rebound arg 7", __LINE__);

  // Enqueue kernel
  size_t global[2] = {params.nx, params.ny};
  size_t local[2] = {bx, by};
