                              ...
                              local int *Bblk)
{
  ...
  int xblocks = nx/blksz;
  int yblocks = ny/blksz;
  ...
  {  
    ...
    Ablk[lx + ly*blksz] = cells[(lx + b*blksz) + (ly + b*blksz)*nx];
    
    /*
    for (int kk = 0; kk < NSPEEDS; kk++)
    {
      Ablk[lx + ly*blksz].speeds[kk] = cells[(lx + b*blksz) + (ly + b*blksz)*nx].speeds[kk];
    }
    
    
    Ablk[lx + ly*blksz].speeds[kk] = cells[(lx + b*blksz) + (ly + b*blksz)*nx].speeds[kk]; */
    Bblk[lx + ly*blksz] = obstacles[(lx + b*blksz) + (ly + b*blksz)*nx];
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int k = 0; k < blksz; k++){
      ...
      for (int kk = 0; kk < NSPEEDS; kk++)
      {
        local_density += Ablk[k + ly*blksz].speeds[kk];
      }

      float u_x = (Ablk[k + ly*blksz].speeds[1] 
        + Ablk[k + ly*blksz].speeds[5] 
        + Ablk[k + ly*blksz].speeds[8] 
        - Ablk[k + ly*blksz].speeds[3] 
        - Ablk[k + ly*blksz].speeds[6]
        - Ablk[k + ly*blksz].speeds[7]) / local_density;
      
      float u_y = (Ablk[k + ly*blksz].speeds[2]
        + Ablk[k + ly*blksz].speeds[5]
        + Ablk[k + ly*blksz].speeds[6]
        - Ablk[k + ly*blksz].speeds[4]
        - Ablk[k + ly*blksz].speeds[7]
        - Ablk[k + ly*blksz].speeds[8]) / local_density;

      sums[0] += sqrt((u_x * u_x) + (u_y * u_y));
      }
    }
     */
  } 
