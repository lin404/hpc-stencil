...
#pragma omp for schedule(static)
    for (int ij = 0; ij < params.nx * params.ny; ij++)
    // for (int jj = 0; jj < params.ny; jj++)
    {
      //   for (int ii = 0; ii < params.nx; ii++)
      //   {
      ...
      // if (!obstacles[ii + jj*params.nx])
      {
        ...
        float u_x = (tmp_cells[ij].speeds[1] + tmp_cells[ij].speeds[5] + tmp_cells[ij].speeds[8] - (tmp_cells[ij].speeds[3] + tmp_cells[ij].speeds[6] + tmp_cells[ij].speeds[7])) / local_density;
        /* compute y velocity component */
        float u_y = (tmp_cells[ij].speeds[2] + tmp_cells[ij].speeds[5] + tmp_cells[ij].speeds[6] - (tmp_cells[ij].speeds[4] + tmp_cells[ij].speeds[7] + tmp_cells[ij].speeds[8])) / local_density;
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          cells[ij].speeds[kk] = tmp_cells[ij].speeds[kk] + params.omega * (d_equ[kk] - tmp_cells[ij].speeds[kk]);
        }
      }
    }
  }
...
#pragma omp parallel for reduction(+ \
                                   : tot_u, tot_cells) private(u_x, u_y)
  for (int ij = 0; ij < params.nx * params.ny; ij++)
  // for (int jj = 0; jj < params.ny; jj++)
  {
    // for (int ii = 0; ii < params.nx; ii++)
    ...
    // if (!obstacles[ii + jj*params.nx])
    {
      ...
      u_x = (cells[ij].speeds[1] + cells[ij].speeds[5] + cells[ij].speeds[8] - (cells[ij].speeds[3] + cells[ij].speeds[6] + cells[ij].speeds[7])) / local_density;
      /* compute y velocity component */
      u_y = (cells[ij].speeds[2] + cells[ij].speeds[5] + cells[ij].speeds[6] - (cells[ij].speeds[4] + cells[ij].speeds[7] + cells[ij].speeds[8])) / local_density;
      /* x-component of velocity */
