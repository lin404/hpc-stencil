  ...
  ** are in the scratch-space grid */
  ...
  //   {
      ...
      // if (!obstacles[ii + jj*params.nx])
      {
        ...
        float u_x = (tmp_cells[ij].speeds[1]
        ...
        float u_y = (tmp_cells[ij].speeds[2]
                      + tmp_cells[ij].speeds[5]
                      + tmp_cells[ij].speeds[6]
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          cells[ij].speeds[kk] = tmp_cells[ij].speeds[kk]
                                                  + params.omega
                                                  * (d_equ[kk] - tmp_cells[ij].speeds[kk]);
        }
      }
    }
  }
...
float av_velocity(const t_param params, t_speed* cells, int* obstacles)
{
  ...
  #pragma omp parallel for reduction(+:tot_u, tot_cells) private(u_x,u_y) 
  for (int ij = 0; ij < params.nx * params.ny; ij++)
  // for (int jj = 0; jj < params.ny; jj++)
  {
    // for (int ii = 0; ii < params.nx; ii++)
